...
class CloneClassifier:
    ...
    def prepare_inputs(self, df: pd.DataFrame):
        ...
        dset = dset.map(fn_preprocessor, batched=True, num_proc=CPU_COUNT, load_from_cache_file=False)

        an_preprocessor = AnnotationPreprocessor()
        dset = dset.map(an_preprocessor, batched=True, num_proc=CPU_COUNT, load_from_cache_file=False)
    ...
    def predict(
        ...
        Returns:
            pd.DataFrame: DataFrame with 'file_name', 'predictions', and 'similarity_score' columns.
        """
        # -- Preparing inputs
    ...
    def get_top_k_similar(self, target_code: str, code_repository: list, file_names: list, k: int = 5) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            file_names (list): List of file names corresponding to the code snippets.
