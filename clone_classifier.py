...
class CloneClassifier:
    """
    A class that integrates data preprocessing, input tokenization, and model
    inference. It takes in a pandas DataFrame with two columns: "code1" and
    "code2", and returns the predictions as a DataFrame with similarity scores.
    """

    PLM = "Lazyhope/python-clone-detection"  # Hugging Face model name

    def __init__(self, max_token_size=512, fp16=False, per_device_eval_batch_size=32):
        """Initialize the CloneClassifier."""
        # -- Tokenizing & Encoding
        self.tokenizer = AutoTokenizer.from_pretrained(self.PLM)
        self.encoder = Encoder(self.tokenizer, max_input_length=max_token_size)
        ...
        training_args = TrainingArguments(
            ...
            per_device_eval_batch_size=per_device_eval_batch_size,
            fp16=fp16,
            remove_unused_columns=True,  # Ensures only necessary columns are passed
        )
    ...
    def prepare_inputs(self, df: pd.DataFrame) -> Dataset:
        """Data preprocessing and tokenization."""
        try:
            # -- Loading datasets
            dset = Dataset.from_pandas(df)
            ...
            # -- Preprocessing datasets
            CPU_COUNT = multiprocessing.cpu_count() // 2
            logger.info(f"Using {CPU_COUNT} processes for preprocessing.")
            ...
            dset = dset.map(
                ...
                num_proc=CPU_COUNT,
                remove_columns=[
                    col for col in dset.column_names if col not in ['input_ids', 'attention_mask']
                ],
            )
    ...
    def predict(
        ...
        Args:
            df (pd.DataFrame): DataFrame containing 'code1' and 'code2' columns.
            save_path (str, optional): Path to save the results as a CSV file. Defaults to None.
            return_first (bool, optional): Whether to return only the first result. Defaults to False.
        ...
        try:
            ...
            # -- Inference
            outputs = self.trainer.predict(dset).predictions  # logits output

            logger.debug(f"Model outputs: {outputs}")
            scores = torch.tensor(outputs)
            if scores.ndim == 1:
                scores = scores.unsqueeze(0)
            scores = scores.softmax(dim=-1).numpy()  # probability output
            logger.debug(f"Softmax scores: {scores}")

            results = df[["code1", "code2"]].copy()
            results["predictions"] = np.argmax(scores, axis=-1)
            results["similarity_score"] = scores[:, 1] * 100  # Score of positive class
    ...
    def get_similarity_score(self, target_code: str, compare_code: str) -> float:
        ...
        result = self.predict(df, return_first=True)
        return result['similarity_score'].iloc[0]
    ...
    def get_top_k_similar(self, target_code: str, code_repository: list, k: int = 5) -> pd.DataFrame:
        ...
        top_k_results = results.nlargest(k, 'similarity_score').reset_index(drop=True)
