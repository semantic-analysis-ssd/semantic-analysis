...
def main():
    ...
    # Step 2: Read all code files from the folder and store file names
    code_repository = []
    file_names = []
    for file_name in os.listdir(folder_path):
        file_path = Path(folder_path) / file_name
    ...
    # Step 4: Add file names to the results
    top_k_results["file_name"] = [
        file_names[idx] for idx in top_k_results.index
    ]  # Map file names to results
    ...
    # Print the results
    print("Top K Similar Codes with File Names:")
    print(top_k_results[["file_name", "similarity_score"]])  # Show file names and scores
